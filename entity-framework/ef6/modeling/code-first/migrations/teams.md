...
**Tento přístup je možné pouze v případě nejnovější migrace existuje pouze v místním kódové základny a dosud nebyla odeslána do správy zdrojového kódu (to znamená poslední migrace byla vygenerována koncipovaná sloučení)**. Úprava metadat migrace, které ostatní vývojáři mohou už jste použili k jejich vývoj databáze – nebo dokonce horší použitý k produkční databázi – může způsobit neočekávané vedlejší účinky. Během procesu budeme vrátit zpět poslední migrace v místní databázi a použijte ho znovu s aktualizovanými metadaty.
